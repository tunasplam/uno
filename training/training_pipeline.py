...
class UnoTrainingPipeline:
    def __init__(self, base_model: str, config: dict, save_dir: str, model_id: str, seed: int=24):
    ...
    def download_base_model(self):
        ...
        os.makedirs(self.save_dir + "/uno", exist_ok=True)

        if os.path.exists(self.save_dir + f"/uno-agent-agent-base-{self.model_id}"):
            print("Base model already exists. Skipping download")
            return

        # you do not need to train the tokenizer
        self.tokenizer = AutoTokenizer.from_pretrained(
            self.base_model, use_fast=False
        )
        self.tokenizer.save_pretrained(self.save_dir + f"/tokenizer-{self.model_id}")

        model = AutoModelForSeq2SeqLM.from_pretrained(
            self.base_model, device_map="auto"
        )
        model.save_pretrained(self.save_dir + f"/uno-agent-base-{self.model_id}")

    def supervised_fine_tuning(self, train_data_fp: str, test_ratio: float=0.1):
        ...
        model.save_pretrained(self.save_dir + f"/uno-agent-sft-{self.model_id}")

    def _create_model(self):
        model = AutoModelForSeq2SeqLM.from_pretrained(
            self.save_dir + f"/uno-agent-base-{self.model_id}"
        )
    ...
    def _config_training_args(self):
        ...
        return TrainingArguments(
            ...
            output_dir=self.save_dir + f"/uno-agent-training-args-{self.model_id}",
            ...
            logging_dir=self.save_dir + f"/logs-{self.model_id}",
    ...
    def _create_data_collator(self, model) -> DataCollatorForSeq2Seq:
        ...
        if not self.tokenizer:
            self.tokenizer = AutoTokenizer.from_pretrained(
                self.save_dir + f"/uno-agent-tokenizer-{self.model_id}"
            )
    ...
    def _load_and_split_data(self, train_data_fp: str, test_ratio: float) -> tuple[Dataset, Dataset]:
        if not self.tokenizer:
            self.tokenizer = AutoTokenizer.from_pretrained(
                self.save_dir + f"/uno/tokenizer-{self.model_id}"
            )
        ...
        print(f"Training using {len(dataset['train'])} entries")
        print("\n\nSample Entry:\n")
        ...
        split_dataset = tokenized_dataset.train_test_split(test_size=test_ratio, seed=self.seed)
        return split_dataset['train'], split_dataset['test']

    def reinforcement_learning(self):
        model = AutoModelForSeq2SeqLM.from_pretrained(
            self.save_dir + f"/uno-agent-sft-{self.model_id}"
        )
        # TODO

if __name__ == '__main__':
    pipeline = UnoTrainingPipeline("google/flan-t5-base", {}, "/home/jordan/agents/uno", "test")
    pipeline.download_base_model()
